...
# Sampling rate, frequency bands, channel list, and PSD frequency range
...
# Returns the processed data directory for the current player; creates it if missing
def processed_root() -> Path:
    d = base_dir() / "player_data" / "processed"
    d.mkdir(parents=True, exist_ok=True)
    return d


# Validates and cleans the labeled EEG DataFrame; discards SAM values outside 1–5
def prepare_epoch_dataframe(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    out = out[out["sam_label"].between(1, 5)].copy()  # Discard invalid SAM labels
...
# Converts the sample-level DataFrame into a 3D tensor (n_epochs, n_channels, n_times)
...
def build_epoch_tensor(df: pd.DataFrame, cfg: Config):
    ...
    for epoch_id, g in df.groupby("epoch", sort=True):
        ...
        # Use event onset time if available; otherwise fall back to first sample time
...
# Computes Power Spectral Density using the multitaper method
...
# Averages PSD values within a frequency band → returns (n_epochs, n_channels)
...
# Runs the full feature extraction pipeline for the current player and returns a flat feature DataFrame
def extract_features_for_current_player(cfg: Config) -> pd.DataFrame:
    pid      = "current_player"
    csv_path = processed_root() / f"{pid}_labeled.csv"

    if not csv_path.exists():
        raise FileNotFoundError(f"Missing labeled file: {csv_path.name}")
...
def extract_all() -> None:
    print("=== Step 3: Feature Extraction (current player) ===")
    cfg = Config()

    try:
        df_feat  = extract_features_for_current_player(cfg)
        out_path = processed_root() / "current_player_features.csv"
        df_feat.to_csv(out_path, index=False)
        print(f"[OK] current_player: {out_path.name} (epochs={len(df_feat):,})")
